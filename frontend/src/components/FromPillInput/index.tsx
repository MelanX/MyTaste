...
const FromPillInput: React.FC<Props> = ({ value, onChange, placeholder }) => {
    ...
    useEffect(() => {
        const onDown = (e: KeyboardEvent) => { if (e.ctrlKey || e.shiftKey) setModifierHeld(true); };
        const onUp = (e: KeyboardEvent) => { if (!e.ctrlKey && !e.shiftKey) setModifierHeld(false); };
            ...
            {value.length > 0 && (
                <div className={styles.pillsRow}>
                    {value.map((pill, i) => (
                        <span
                            key={i}
                            className={`${styles.pill}${modifierHeld ? ` ${styles.pillDanger}` : ''}`}
