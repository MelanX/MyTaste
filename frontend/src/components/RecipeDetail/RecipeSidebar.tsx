          ...
          <div className={styles.sidebarImageContainer}>
            {isAuthenticated && (
              <div className={styles.buttonRow}>
                <div onClick={handleToggleCook} className={`${styles.cookIcon} no-print`}>
                  {status.cookState ? (
                    <i className="fa-solid fa-check-circle" title="Bereits gekocht" />
                  ) : (
                    <i className="fa-solid fa-question" title="Noch nicht gekocht" />
                  )}
                </div>
                <div className={`${styles.favIcon} no-print`} onClick={handleToggleFavorite}>
                  {recipe.status?.favorite ? (
                    <i className="fa-solid fa-heart" title="Favorit" />
                  ) : (
          ...
          {recipe.ingredient_sections && recipe.ingredient_sections.length > 0 ? (
            ...
            recipe.ingredient_sections.map((section, sectionIndex) => (
              <div key={sectionIndex} className={styles.ingredientsCard}>
                {section.title ? <h3 className={styles.ingredientSectionTitle}>{section.title || 'Zutaten'}</h3> : <></>}
                    ...
                    return (
                      <div key={index} className={styles.ingredientRow}>
                        ...
                        <div className={styles.ingredientName}>
                          ...
                          {nameSpecification && <span className={styles.nameSpecification}>{nameSpecification}</span>}
                        </div>
                        {ingredient.note && (
