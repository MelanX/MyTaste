...
const CollectionCard: React.FC<Props> = ({ collection, recipes }) => {
    ...
    const imageSrc = first?.image
        ? (first.image.startsWith('/uploads') ? `${ getConfig().API_URL }${ first.image }` : first.image)
        : '/placeholder.webp';
        ...
        <div className={ styles.card }>
            <Link to={ `/collections/${ collection.id }` } className={ styles.imageLink }>
                ...
                <div className={ styles.overlay }>
                    ...
                    <span
                        className={ styles.count }>{ collection.recipeIds.length } Rezept{ collection.recipeIds.length !== 1 ? 'e' : '' }</span>
                </div>
            </Link>
        </div>
    );
