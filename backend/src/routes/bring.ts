...
import { readData, readImportConfig } from '../utils/fileService.js';
import type { BringRule } from '../utils/fileService.js';

function normalizeIngredient(name: string, bringRules: BringRule[]): string {
...
router.get('/bring-recipe/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const items: BringItem[] = [
      ...[...merged.values()].map((i) => {
        const item: BringItem = { itemId: i.itemId };
        if (i.amount != null) {
          item.spec = `${String(i.amount).replace('.', ',')} ${i.unit || ''}`.trim();
        }
        return item;
      }),
      ...(recipe.spices || []).map((s): BringItem => ({ itemId: s, stock: true })),
    ];
...
router.get('/bring-bulk', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const items: BringItem[] = [
      ...
      ...[...spiceSet].map((s): BringItem => ({ itemId: s, stock: true })),
    ];
