...
beforeEach(() => {
  ...
  fs.__setCollectionsData({ nextUp: [], collections: [] });
});
...
describe('GET /api/collections/next-up', () => {
  ...
  it('returns current IDs after adds', async () => {
    fs.__setCollectionsData({ nextUp: ['r1'] } as never);
    const res = await agent.get('/api/collections/next-up');
    expect(res.status).toBe(200);
...
describe('POST /api/collections/next-up/:id', () => {
  ...
  it('returns 403 with bad token', async () => {
    const res = await agent.post('/api/collections/next-up/r1').set('Authorization', 'Bearer badtoken');
  ...
  it('returns 404 when the recipe ID does not exist', async () => {
    const res = await agent.post('/api/collections/next-up/nonexistent').set(authHeader());
...
describe('DELETE /api/collections/next-up/:id', () => {
  ...
  it('returns 403 with bad token', async () => {
    const res = await agent.delete('/api/collections/next-up/r1').set('Authorization', 'Bearer badtoken');
    expect(res.status).toBe(403);
  });

  it('removes the ID and returns updated list', async () => {
    const res = await agent.delete('/api/collections/next-up/r1').set(authHeader());
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ nextUp: [] });
  });

  it('returns 404 when the ID is not in the list', async () => {
    const res = await agent.delete('/api/collections/next-up/notinlist').set(authHeader());
    expect(res.status).toBe(404);
  });
});
...
describe('DELETE /api/collections/next-up', () => {
  beforeEach(() => {
    fs.__setCollectionsData({ nextUp: ['r1'], collections: [] });
  });
  ...
  it('returns 403 with bad token', async () => {
    const res = await agent.delete('/api/collections/next-up').set('Authorization', 'Bearer badtoken');
    expect(res.status).toBe(403);
  });

  it('clears the list and returns empty nextUp', async () => {
    const res = await agent.delete('/api/collections/next-up').set(authHeader());
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ nextUp: [] });
  });
});
...
describe('Named Collections', () => {
  describe('GET /api/collections', () => {
    ...
    it('returns existing collections', async () => {
      fs.__setCollectionsData({ nextUp: [], collections: [seedCollection()] });
      const res = await agent.get('/api/collections');
      expect(res.status).toBe(200);
  ...
  describe('POST /api/collections', () => {
    ...
    it('returns 403 with bad token', async () => {
      const res = await agent.post('/api/collections').send({ name: 'Test' }).set('Authorization', 'Bearer badtoken');
      expect(res.status).toBe(403);
    });
  ...
  describe('PATCH /api/collections/:id', () => {
    ...
    it('returns 403 with bad token', async () => {
      const res = await agent.patch('/api/collections/c1').send({ name: 'New' }).set('Authorization', 'Bearer badtoken');
      expect(res.status).toBe(403);
    });
  ...
  describe('DELETE /api/collections/:id', () => {
    ...
    it('returns 403 with bad token', async () => {
      const res = await agent.delete('/api/collections/c1').set('Authorization', 'Bearer badtoken');
      expect(res.status).toBe(403);
    });
  ...
  describe('POST /api/collections/:id/recipes/:recipeId', () => {
    ...
    it('returns 403 with bad token', async () => {
      const res = await agent.post('/api/collections/c1/recipes/r1').set('Authorization', 'Bearer badtoken');
      expect(res.status).toBe(403);
    });
  ...
  describe('DELETE /api/collections/:id/recipes/:recipeId', () => {
    beforeEach(() => {
      fs.__setCollectionsData({ nextUp: [], collections: [seedCollection({ recipeIds: ['r1'] })] });
    });
    ...
    it('returns 403 with bad token', async () => {
      const res = await agent.delete('/api/collections/c1/recipes/r1').set('Authorization', 'Bearer badtoken');
      expect(res.status).toBe(403);
    });
  ...
  describe('DELETE /api/collections/:id/recipes', () => {
    ...
    it('returns 403 with bad token', async () => {
      const res = await agent.delete('/api/collections/c1/recipes').set('Authorization', 'Bearer badtoken');
      expect(res.status).toBe(403);
    });
