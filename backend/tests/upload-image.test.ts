...
describe('POST /api/upload-image', () => {
  ...
  it('403 with invalid credentials', async () => {
    const res = await agent.post('/api/upload-image').set('Authorization', 'Bearer invalid').send({});
  ...
  it('400 when no file is sent', async () => {
    const res = await agent.post('/api/upload-image').set(authHeader()).send({}); // no multipart/form-data
  ...
  it('400 when wrong field-name is used', async () => {
    const res = await agent.post('/api/upload-image').set(authHeader()).field('not_file', 'oops').attach('not_file', Buffer.from('fake'), {
      filename: 'fake.png',
  ...
  it('413 when file too large', async () => {
    ...
    const res = await agent.post('/api/upload-image').set(authHeader()).attach('file', large, {
      filename: 'big.png',
      contentType: 'image/png',
    });
  ...
  it('200 and returns a .webp URL on success', async () => {
    ...
    const res = await agent.post('/api/upload-image').set(authHeader()).attach('file', tiny, {
      filename: 'pixel.png',
      contentType: 'image/png',
    });
