...
describe('Auth flow', () => {
  ...
  it('401 on wrong credentials', async () => {
    const res = await agent.post('/api/login').send({ username: 'foo', password: 'bar' });
  ...
  it('401 on invalid refresh token', async () => {
    const res = await request(app).post('/api/refresh').set('Cookie', 'refresh_token=bad.token.here');
    expect(res.status).toBe(401);
  });
