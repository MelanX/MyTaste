...
import { recipeSchema } from '../src/utils/schemes.js';
...
describe('recipeSchema – recipeType field', () => {
  const valid = (body: Record<string, unknown>) => recipeSchema.validate(body, { abortEarly: false });
...
describe('recipeSchema – dietaryRestrictions field', () => {
  ...
  it.each(['vegan', 'vegetarian', 'glutenfree', 'dairyfree'])('accepts a single-item array with "%s"', (d) => {
...
describe('POST /api/recipes – recipeType and dietaryRestrictions', () => {
  ...
  it('creates a recipe without recipeType (field absent from response)', async () => {
    const res = await agent.post('/api/recipes').set(authHeader()).send(base);
