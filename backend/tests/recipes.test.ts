    ...
    recipes: [
      {
        ...
        spices: ['Salz'],
        instructions: ['Iss einfach die Schokolade'],
...
describe('Recipes endpoints', () => {
  ...
  it('GET /bring-recipe converts data for Bring! app', async () => {
    ...
    expect(res.body.items.some((i: { itemId: string; stock?: boolean }) => i.itemId === 'Salz' && i.stock)).toBe(true);
  });
  ...
  it('POST /recipes with bad token → 403', async () => {
    const res = await agent.post('/api/recipes').set('Authorization', 'Bearer badtoken').send({ title: 'x' });
  ...
  it('POST /recipes succeeds when authorised', async () => {
    ...
    const res = await agent.post('/api/recipes').set(authHeader()).send(body);
  ...
  it('Rejects recipe without title', async () => {
    const res = await agent.post('/api/recipes').set(authHeader()).send({ url: 'https://ex.com' });
...
describe('DELETE /recipe/:id', () => {
  ...
  it('403s with a bad token', async () => {
    const res = await request(app).delete('/api/recipe/1').set('Authorization', 'Bearer invalidtoken');
    expect(res.status).toBe(403);
  });
  ...
  it('204s and removes the recipe when authorised', async () => {
    ...
    // delete it
    const deleteRes = await request(app).delete('/api/recipe/1').set(authHeader());
  ...
  it('404s for a non-existent id even when authorised', async () => {
    const res = await request(app).delete('/api/recipe/does-not-exist').set(authHeader());
    expect(res.status).toBe(404);
  });
  ...
  it('GET /recipes list shrinks after deleting a recipe', async () => {
    ...
    const createRes = await request(app).post('/api/recipes').set(authHeader()).send(newRecipe);
    ...
    const delRes = await request(app).delete(`/api/recipe/${createdId}`).set(authHeader());
  ...
  it('cannot delete twice (second delete → 404)', async () => {
    ...
    const firstDel = await request(app).delete(`/api/recipe/${id}`).set(authHeader());
    expect(firstDel.status).toBe(204);

    // second delete should 404
    const secondDel = await request(app).delete(`/api/recipe/${id}`).set(authHeader());
  ...
  it('DELETE /recipe/:id does not accidentally delete other recipes', async () => {
    ...
    const delA = await request(app).delete(`/api/recipe/${idA}`).set(authHeader());
  ...
  it('removes deleted recipe from nextUp', async () => {
    fs.__setCollectionsData({ nextUp: ['1'], collections: [] });
  ...
  it('removes deleted recipe from named collections', async () => {
    fs.__setCollectionsData({
      nextUp: [],
      collections: [{ id: 'c1', name: 'Test', recipeIds: ['1', '99'], createdAt: '', updatedAt: '' }],
    });
    await request(app).delete('/api/recipe/1').set(authHeader());
    const res = await request(app).get('/api/collections');
    const col = res.body.collections.find((c: { id: string }) => c.id === 'c1');
...
describe('GET /api/recipes sorting', () => {
  it('puts favorites first, then cooked, then alphabetical by title', async () => {
    ...
    // Assert
    expect(res.status).toBe(200);
    expect(res.body.recipes.map((r: { id: string }) => r.id)).toEqual([
      '4', // favorite & cooked
      '2', // favorite only
      '3', // cooked only
      '1', // neither
...
describe('PUT /api/recipe/:id', () => {
  ...
  it('403 with an invalid token', async () => {
    const res = await agent.put('/api/recipe/1').set('Authorization', 'Bearer bad.token.here').send(validBody);
    expect(res.status).toBe(403);
  });

  it('404 when the recipe does not exist', async () => {
    const res = await agent.put('/api/recipe/not-there').set(authHeader()).send(validBody);
    expect(res.status).toBe(404);
  });
  ...
  it('400 on validation error (e.g. missing title)', async () => {
    ...
    void title;
    const res = await agent.put('/api/recipe/1').set(authHeader()).send(invalidBody);
    expect(res.status).toBe(400);
  });

  it('200 updates the recipe and returns the new data', async () => {
    const res = await agent.put('/api/recipe/1').set(authHeader()).send(validBody);
...
describe('PATCH /api/recipe/:id/status', () => {
  ...
  it('400 if status is missing', async () => {
    await request(app).patch('/api/recipe/1/status').set(authHeader()).send({}).expect(400);
  });
});
