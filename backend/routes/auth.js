...
router.post('/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    const accessToken = jwt.sign({ user: value.username }, JWT_SECRET, { expiresIn: '15m' });

    const refreshToken = jwt.sign({ user: value.username }, process.env.JWT_REFRESH_SECRET || JWT_SECRET, {
      expiresIn: '30d',
      jwtid: randomUUID(),
    });
...
router.post('/refresh', async (req, res) => {
  ...
  jwt.verify(old, process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET, async (err, decoded) => {
    ...
    const newRefreshToken = jwt.sign({ user: decoded.user }, process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET, {
      expiresIn: '30d',
      jwtid: randomUUID(),
    });
    const accessToken = jwt.sign({ user: decoded.user }, process.env.JWT_SECRET, { expiresIn: '15m' });
...
router.post('/logout', async (req, res) => {
  ...
  if (refreshToken) {
    try {
      const { jti, exp } = jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET);
